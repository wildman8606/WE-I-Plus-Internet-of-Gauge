...
int main(void)
{
    ...
    while (true)
    {
        ...
        for (uint32_t i = 0; i < EI_CLASSIFIER_OBJECT_DETECTION_COUNT; i++) {
        ...
        #if 1
            algoresult.ht[i].upper_body_label = (*bb.label)-48;
            ...
            hx_drv_uart_print(" %d score: (%u) [ x: %u, y: %u]\r\n", 
                    algoresult.ht[i].upper_body_label,
                    algoresult.ht[i].upper_body_score, 
        ...
        if (bbox_quantity>0)
        {
			...
			int data = algoresult.ht[0].upper_body_label;
			if (bbox_quantity>2){
				for (uint32_t i = 1; i < bbox_quantity; i++)
					data = data*10 + algoresult.ht[i].upper_body_label;
					hx_drv_uart_print("[%d]\r\n", data);
